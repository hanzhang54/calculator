...
namespace MyScriptEditor.UserControls
{
    ...
    public sealed partial class EditorUserControl : IRenderTarget
    {
        ...
        private void Capture_PointerPressed(object sender, PointerRoutedEventArgs e)
        {
            ...
            try
            {
                ...
                _lastPointerPosition = new MyScript.IInk.Graphics.Point((float)p.Position.X, (float)p.Position.Y);
                _onScroll = false;
        ...
        public void ResetView(bool forceInvalidate)
        {
            ...
            renderer.ViewOffset = new MyScript.IInk.Graphics.Point(0, 0);

            // Get new view transform (keep only scale and offset)
            var tr = renderer.GetViewTransform();
            tr = new MyScript.IInk.Graphics.Transform(tr.XX, tr.YX, 0, tr.XY, tr.YY, 0);
            ...
            if (Editor.Part.Type == "Raw Content")
            {
                ...
                var viewCenter = new MyScript.IInk.Graphics.Point(Editor.ViewWidth * 0.5f, Editor.ViewHeight * 0.5f);
